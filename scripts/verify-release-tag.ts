...
const rustVersion = cargoWorkspaceVersion(await readFile(path.join(repositoryRoot, "rust", "Cargo.toml"), "utf8"));
if (rustVersion !== packageJson.version) {
  throw new Error(`Rust workspace version ${rustVersion} does not match extension version ${packageJson.version}`);
}
